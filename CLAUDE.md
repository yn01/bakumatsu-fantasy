...
- **完全版範囲**: 土佐編〜近江屋事件（龍馬の全生涯）
- **プレイ時間**: 2〜4時間（メインストーリー）
...
- **コンテキスト節約のため、サブエージェントへの委託を徹底**すること
  ...
  - **モデルの使い分け**: Orchestrator（メイン会話）は **Opus**、サブエージェントは **Sonnet** を使用すること
    - Agent tool 呼び出し時に `model: "sonnet"` を明示する
    - 理由: 委託の回数が多くなるためコスト効率を優先。指揮・判断を担うメイン会話のみ Opus の推論能力が必要
...
**実装期間**: 2026-02-15
**完了タスク**: 8タスク（Task #48-#66）
**進捗率**: 100%
**最終バンドルサイズ**: 234.13 kB (gzip: 72.72 kB)

**主な機能**:
1. 新キャラクター5人（勝海舟、西郷隆盛、木戸孝允、中岡慎太郎、お龍）
2. 新スキル~20種（海軍/薩摩/政治家/志士/サポーター5系統）
3. 新スキルツリー5クラス（naval_commander, satsuma_warrior, politician, shishi, supporter）
4. 新アイテム20種（武器8、防具6、消費6、Lv8-25帯）
5. 新敵25種（新選組10、幕府10、暗殺者5）
6. 新マップ3つ（神戸港45x35、京都60x50、近江屋20x15）
7. 新シナリオ5章（第4章〜最終章）+ エピローグ改修
8. エンディング2分岐（勝利：生存ルート / 敗北：史実ルート）

**データ追加量**:
| カテゴリ | ファイル | 変更内容 |
|----------|---------|---------|
| キャラクター | characters.json | 5人追加（katsu, saigo, kido, nakaoka, oryo） |
| スキル | skills.json | ~20スキル追加 |
| スキルツリー | skill_trees.json | 5新クラスツリー追加 |
| アイテム | items.json | 20アイテム追加 |
| 敵 | enemies.json | 25敵追加（合計39種） |
| マップ | kobe_port.json | 45x35、NPC6、ショップ2 |
| マップ | kyoto.json | 60x50、NPC10、ショップ3 |
| マップ | omiya.json | 20x15、NPC3、エンカウントなし |
| シナリオ | chapter4.json | 神戸海軍操練所（~6KB） |
| シナリオ | chapter5.json | 薩長同盟（~8KB） |
| シナリオ | chapter6.json | 大政奉還工作（~6KB） |
| シナリオ | chapter7.json | 新政府構想（~7KB） |
| シナリオ | final_chapter.json | 近江屋事件（~9KB） |
| シナリオ | epilogue.json | 土佐編エピローグに改修 |

**コード変更**:
- ScenarioManager.ts: chapter4〜final_chapterのフラグチェック・マップイベントトリガー追加
- EventManager.ts: 5シナリオファイルをscenarioFilesに追加
- TitleScreen.tsx: バージョンv2.0.0に更新
- urado_port.json: kobe_portへの遷移追加
- kochi_town.json: 城門通路のcollision修正

**バンドルサイズ推移**:
- Phase 7完了: 233.10 kB (gzip: 72.52 kB)
- **Phase 8完了: 234.13 kB (gzip: 72.72 kB) → +1.03 kB (gzip: +0.20 kB)**

**Phase 8 完了**: 完全版シナリオ実装完了、龍馬の全生涯を描くストーリー完成！

### Phase 9 完了サマリー

**実装期間**: 2026-02-15
**完了タスク**: 5タスク（Task #48-#52）
**進捗率**: 100%
**最終バンドルサイズ**: 241.60 kB (gzip: 74.65 kB)

**主な機能**:
1. サイドクエスト20個（全マップに配置、クエストUI、進捗管理）
2. 隠しダンジョン「幕末の亡霊たち」（ボス5体、究極装備6種）
3. 実績システム（30種類、達成率表示）
4. 図鑑システム（敵/アイテム/スキル3種）
5. New Game+（引き継ぎ、2周目敵強化1.3倍）
6. 難易度選択（Easy/Normal/Hard/Very Hard）
7. ショップNPC統合（10店舗、地域別商品、条件付きレアアイテム）

**タスク構成**:
| Task | 名称 | 担当チーム | ステータス |
|------|------|------------|------------|
| **#48** | **サイドクエスト実装** | Team Alpha | ✅ 完了 |
| **#49** | **隠しダンジョン実装** | Team Beta | ✅ 完了 |
| **#50** | **やり込み要素実装** | Team Beta | ✅ 完了 |
| **#51** | **難易度選択実装** | Team Beta | ✅ 完了 |
| **#52** | **ショップNPC統合** | Team Alpha | ✅ 完了 |

**バンドルサイズ推移**:
- Phase 8完了: 234.13 kB (gzip: 72.72 kB)
- **Phase 9完了: 241.60 kB (gzip: 74.65 kB) → +7.47 kB (gzip: +1.93 kB)**

**Phase 9 完了**: ゲームシステム拡張完了、やり込み要素充実！

**詳細**: `docs/PHASE9_ACTUALS.md` を参照

### Phase 10 完了サマリー

**実装期間**: 2026-02-17
**完了タスク**: 4タスク（Task #53-#56）
**進捗率**: 100%
**最終バンドルサイズ**: 274.93 kB (gzip: 83.98 kB)

**主な機能**:
1. 統合入力システム（キーボード/ゲームパッド/マウス対応、InputManager）
2. プロシージャルドット絵スプライト生成（SpriteGenerator、7キャラ+NPC+敵）
3. 和風タイルセット生成（TilesetGenerator、10種のタイル）
4. バトル背景生成（4種: 城下町、山道、海岸、屋内）
5. アニメーションシステム（AnimationManager、BattleEffects）
6. 設定画面（BGM/SE音量、メッセージ速度、ウィンドウスキン、フルスクリーン）

**タスク構成**:
| Task | 名称 | 担当チーム | ステータス |
|------|------|------------|------------|
| **#53** | **操作性改善（InputManager）** | Team Beta | ✅ 完了 |
| **#54** | **ビジュアル強化（SpriteGenerator）** | Team Alpha | ✅ 完了 |
| **#55** | **アニメーション追加** | Team Alpha | ✅ 完了 |
| **#56** | **設定画面実装** | Team Beta | ✅ 完了 |

**バンドルサイズ推移**:
- Phase 9完了: 241.60 kB (gzip: 74.65 kB)
- **Phase 10完了: 274.93 kB (gzip: 83.98 kB) → +33.33 kB (gzip: +9.33 kB)**

**Codexレビュー結果** (2026-02-17):
- 検出問題: Critical 1件、High 3件、Medium 4件、Low 3件
- **Critical優先度1件修正完了**:
  1. ✅ InputManager単一スロットコールバック → サブスクライバーモデルに変更
- **High優先度3件修正完了**:
  2. ✅ ゲームパッドポーリング重複 → InputManager集中ポーリング（参照カウント）
  3. ✅ ゲームパッドリリース未追跡 → previousGamepadPressedで追跡
  4. ✅ Field.txのstale closure → コールバックをref化
...
| Gamepad B | キャンセル |
| Gamepad Start | メニュー |
| Gamepad D-pad/左スティック | 移動 |

## 完全版コンテンツ
- **プレイアブルキャラ**: 坂本龍馬、武市半平太、勝海舟、西郷隆盛、木戸孝允、中岡慎太郎、お龍
- **マップ**: 才谷屋、高知城下町、山内容堂道場、浦戸湊、神戸港、京都、近江屋
- **敵勢力**: 上士・役人、藩の守旧派、山賊・浪人、新選組、幕府軍、暗殺者
